...
class Personnel extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('designation, username, password', 'required'),
			array('designation, username', 'length', 'max'=>30),
			...
			array('id, designation, username, password, date_created, last_login', 'safe', 'on'=>'search'),
		);
	}
    ...
    public static function sqlPersonnel($attribute){
        $sql = "SELECT username FROM tbl_personnel WHERE username = :username LIMIT 1";
        $command = Yii::app()->db->createCommand($sql);
        $command->bindValue(":username", $attribute, PDO::PARAM_STR);
        return $command->queryRow();
    }

    public static  function sqlPersonnelId($attribute){
        $sql = "SELECT * FROM tbl_personnel WHERE id = :id LIMIT 1;";
        $command = Yii::app()->db->createCommand($sql);
